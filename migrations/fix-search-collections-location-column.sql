-- Fix "column p.location does not exist" (Postgres 42703), surfaced now
-- that fix-duplicate-search-collections-overload.sql removed the
-- ambiguous duplicate function and this one actually runs.
--
-- fix-search-include-own-collections.sql referenced a `location` column
-- on `pins` that has never existed - the real column is `city`
-- (added by add-city-location-fields.sql, and already used the same way
-- by add-social-feed-phase2.sql's city-search function).
